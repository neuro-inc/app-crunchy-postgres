from typing import Any, List, Type, TypeVar, get_origin
...
def find_instances_recursive(
    ...
    if isinstance(obj, BaseModel):
        for field_name, field_value in obj:
            ...
            results.extend(find_instances_recursive(field_value, target_type, visited, field_path))

    # Handle lists and tuples
    elif isinstance(obj, (list, tuple)):
        for idx, item in enumerate(obj):
            item_path = f"{_path}[{idx}]" if _path else f"[{idx}]"
            results.extend(find_instances_recursive(item, target_type, visited, item_path))

    # Handle dictionaries
    elif isinstance(obj, dict):
        for key, value in obj.items():
            dict_path = f"{_path}['{key}']" if _path else f"['{key}']"
            results.extend(find_instances_recursive(value, target_type, visited, dict_path))

    # Handle sets
    elif isinstance(obj, set):
        for idx, item in enumerate(obj):
            set_path = f"{_path}{{item_{idx}}}" if _path else f"{{item_{idx}}}"
            results.extend(find_instances_recursive(item, target_type, visited, set_path))
